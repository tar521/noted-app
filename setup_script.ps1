# setup.ps1
   ...
   # 1. Admin Check
   if (-not
   ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principa
   ...
   choco upgrade nginx -y

   # 4. Project Install
   Write-Host "📂 Installing NPM dependencies..." -ForegroundColor Blue
   npm install && (Set-Location backend; npm install; Set-Location ..) && (Set-Location frontend; npm install;
   Set-Location ..)
   ...
   if ($ConfigContent -notmatch "include servers/\*;") {
       Write-Host "🔧 Linking 'servers' directory in Nginx config..." -ForegroundColor Yellow
       $ConfigContent = $ConfigContent -replace 'http \{', "http {`n    include servers/*;"
       $ConfigContent | Out-File $NginxConf -Encoding ascii
   }
